...
gpu = True
export_results = True
resroot = 'results/for_manuscript/realdata_study_' # where to store results
appendix = '' # to further customize experiment name
experiments = ['ship', 'neutro', 'sarcos', 'tidal']
models_to_run = ['ICM', 'oilmm', 'PLMC', 'PLMC_fast']
if len(models_to_run) == 1:
    appendix += models_to_run[0]
v_test_2 = 'void'
##----------------------------------------------
...
def run_models(models_to_run, models_with_sched, q, lik_rank, n_tasks, n_points, X, Y, X_test, Y_test, lrs, 
                ...
                kernel_type=None, lambda_lr=None, criterion='max', ker_kwargs={}):
    ...
    if 'ICM' in models_to_run:
        models['ICM'] = MultitaskGPModel(X, Y, init_lmc_coeffs=True,
    ...
    if 'PLMC' in models_to_run:
        models['PLMC'] = ProjectedGPModel(X, Y, n_tasks, q, proj_likelihood=None,
    ...
    if 'oilmm' in models_to_run:
        models['oilmm'] = ProjectedGPModel(X, Y, n_tasks, q, proj_likelihood=None, bulk=False,
                                        mean_type=Mean, kernel_type=kernel, decomp=decomp,
    ...
    if 'PLMC_fast' in models_to_run:
        models['PLMC_fast'] = ProjectedGPModel(X, Y, n_tasks, q, proj_likelihood=None,
                                        mean_type=Mean, kernel_type=kernel, decomp=decomp,
    ...
    for name in models_to_run:
        ...
        start = time.time()
        plateau_id = 0
        last_losses[name] = 1e-9
        deltas = np.zeros(patience)
        for i in range(n_iters[name]):
            optimizers[name].zero_grad()
            with gp.settings.cholesky_max_tries(9):
            ...
            if criterion=='max':
                ...
                for j in range(patience - 1):
                    deltas[j+1] = deltas[j]
                deltas[0] = np.abs( 1 - new_loss / last_losses[name])
...
if experiment=='tidal':
    ...
    if __name__ == "__main__":
        ...
        appendix += '' # to further customize experiment name
        path = resroot+ experiment + '_' + appendix + '_' + v_test_0 + '_'+ v_test_1 + '_' + v_test_2 + '.csv'
        print(path + '\n')
        results = {}
        min_err = 1.
...
if experiment=='ship':
    ...
    if __name__ == "__main__":
        ...
        if n_ind_points is not None:
            appendix += '_{0}ind'.format(n_ind_points)
        appendix += ''
        path = resroot+ experiment + '_' + appendix + '_' + v_test + '_'+ v_test_2 + '.csv'
        print(path + '\n')
        results = {}
        for i_v, vval in enumerate(v_vals[v_test]):
            for i_v2, vval2 in enumerate(v_vals[v_test_2]):
                ...
                results, models = run_models(models_to_run, models_with_sched, q, lik_rank, n_tasks, n_points, X, Y, X_test, Y_test, lrs, 
                        n_iters, lr_min, loss_thresh, patience, print_metrics, print_loss, freq_print, gpu, 
                        train_ind_rat, n_ind_points, run_key, results, mean_type=mean_type, lambda_lr=lambda_f, criterion='max')
...
if experiment=='sarcos':
    ...
    if __name__ == "__main__":
        ...
        loss_thresh = 1e-7
        patience = 500
        lrs = dict(zip(models_to_run, [1e-2]*len(models_to_run)))
        n_iters = dict(zip(models_to_run, [10]*len(models_to_run)))
        models_with_sched = models_to_run
        v = {
            'q': n_tasks, 
            'lik_rank': 0,
